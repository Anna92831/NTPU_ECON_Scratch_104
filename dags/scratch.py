...
class JobScraper:
    ...
    def save_to_mysql(self, df):
        if df.empty:
            ...
            return  # Ensure we return here if no data is present
        try:
            ...
            MYSQL_PORT = '3306'
            MYSQL_USER = 'root'
            #MYSQL_PASSWORD = 'qp8=FLgIcgGH|H)B'
            MYSQL_DB = 'airflow'
            MYSQL_PASSWORD = quote_plus('qp8=FLgIcgGH|H)B')
            db_url = f'mysql+pymysql://{MYSQL_USER}:{MYSQL_PASSWORD}@localhost/{MYSQL_DB}?unix_socket=/tmp/mysql.sock'
            self.engine = create_engine(db_url, pool_pre_ping=True, connect_args={"cursorclass": pymysql.cursors.DictCursor})
            logging.info(db_url)
            try:
            ...
            mapped_df.to_sql('jobs', con=self.engine, if_exists='append', index=False)
            logging.info("Successfully saved data to MySQL.")
        except Exception as e:
            logging.error(f"Error saving to MySQL: {str(e)}")
